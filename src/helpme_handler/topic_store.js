...
class TopicStore {
    ...
    constructor() {
        ...
        setInterval(() => {
            ...
            this.topic_creation_score = Math.max(this.topic_creation_score - decrease_amout, 0);
        }, interval_delay);
    }
    /**
     *
    ...
    get_best_matching_topic(message) {
        ...
        const [topic, score] = this.topics
            ...
            .reduce(([acc_topic, acc_score], [curr_topic, curr_score]) => acc_score > curr_score
            ? [acc_topic, acc_score]
            : [curr_topic, curr_score], [first_topic, -Infinity]);
        return {
            topic,
            score,
        };
    ...
    set_topic_for_user(user_id, topic) {
        if (!user_id || !topic) {
            return;
        }
