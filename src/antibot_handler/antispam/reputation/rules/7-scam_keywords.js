...
class ScamKeywordsDetection extends rule_1.BaseMessageReputationRule {
    process(message, current, previous, author_member, pending) {
        ...
        pending.append_if(!normal_delta && scam_word_count > 0, "Author has sent multiple message with a small delta, and message contains scam-y words", -1);
        pending.append_if(!author_has_role && !normal_delta && scam_word_count > 0, "Author has sent multiple message with a small delta, and message contains scam-y words (no role)", -1);
