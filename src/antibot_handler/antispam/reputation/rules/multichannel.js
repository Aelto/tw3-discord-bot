...
exports.SpamDeltaDetection = void 0;
const pending_reputation_1 = require("../pending_reputation");
const rule_1 = require("../rule");
class SpamDeltaDetection extends rule_1.BaseMessageReputationRule {
    process(message, current, previous, author_member, pending) {
        ...
        pending.setVar(pending_reputation_1.ReputationRuleResultKey.PreviousMessageDelta, delta);
        const same_channel = (previous?.channel_id ?? 0) !== 0 &&
            previous.channel_id === current.channel_id;
        pending.setVar(pending_reputation_1.ReputationRuleResultKey.PreviousMessageSameChannel, same_channel);
        ...
        pending.append_if(same_content && !same_channel && is_delta_small_very, "Identical messages sent across multiple channels (very small delta)", -5);
        pending.append_if(same_content && !same_channel && is_delta_tiny, "Identical messages sent across multiple channels (tiny delta)", -5);
        pending.append_if(same_content && is_delta_small && has_link, "Message is identical to a previous message, contains a link, and was sent repeatedly in the last 15 seconds", -2);
        pending.append_if(is_delta_small && mentions_someone, "Message mentions someone and the author is sending many messages in the last 30 seconds", -2);
    }
}
exports.SpamDeltaDetection = SpamDeltaDetection;
