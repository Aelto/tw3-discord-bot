...
class AttachmentsDetection extends rule_1.BaseMessageReputationRule {
    process(message, current, previous, author_member, pending) {
        ...
        // lot of reputation.
        pending.append_if(!has_role && message.attachments.size > 1, "User has no role and message contains 2 or more attachments", -3 * powered_attachments_count);
        pending.append_if(!has_role && message.attachments.size > 0, "User has no role and message contains attachments", -3 * powered_attachments_count);
        ...
        pending.append_if(!has_role && message_is_empty && message.attachments.size > 0, "user has no role and message is short or empty but contains attachments", -1 * powered_attachments_count);
