...
class PositiveGainsDetection extends rule_1.BaseMessageReputationRule {
    process(message, current, previous, author_member, pending) {
        const [author_has_role, is_first_message, has_link, is_delta_normal, is_same_channel,] = pending.getVars([
            ...
            pending_reputation_1.ReputationRuleResultKey.PreviousMessageSameChannel,
        ]);
        ...
        pending.append_if(is_same_channel, "Author sent many messages in a single channel", 0.25);
        pending.append_if(is_same_channel && is_delta_normal, "Author sent many messages in a single channel (with normal or longer delta)", +0.25);
