...
module.exports = async function antibot_handler(message, client) {
  ...
  const contains_allowed_domain = allowed_domains.some((domain) =>
  ...
  if (contains_link && !contains_allowed_domain) {
    ...
    if (has_shut_role || !has_any_role || contains_word_nitro) {
      try {
        ...
        if (!has_shut_role) {
          await author_member.guild.channels.cache
            .get(GRAVEYARD_CHANNEL_ID)
            .send(
              `
...
Thanks for your understanding.`.trim()
      ...
      try {
        ...
        log_channel
          .send(
            `<@${message.author.id}> tried to send a link which was automatically deleted: \`${message.content}\``
          )
          .catch(console.error);
      } catch (err) {}
    }
