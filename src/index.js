...
client.login(key);
const { prompt, prompt_handler, remove_waiting_answer, } = require("./core/prompt.js");
...
commands["help"] = {
    ...
    command: (client, message) => {
        ...
        consume(client, message, "Here is a list of the commands and what they do", helpMessage, "blue");
    },
};
commands["report"] = {
    name: "report",
    ...
    command: async (client, message, args) => {
        ...
        if (!has_admin_role) {
            return consume(client, message, "Missing permissions", "You don't have the sufficient role to report someone", "red");
        }
        let reported_user = null;
        const [at_ping, ...reason_words] = args;
        if (!at_ping) {
            const who_answer = await prompt(client, message, `Looks like you forgot who to report, who do you want to report. @ him please.`, delay, true);
        ...
        try {
            let confirm;
            if (!reported_member.roles.cache.has(WARNED_ROLE_1_ID) &&
                !reported_member.roles.cache.has(WARNED_ROLE_2_ID)) {
                confirm = await prompt(client, message, `Do you confirm you want to report ${reported_user.username}, this will be his 1st offense`, delay);
                level = 1;
            }
            else if (reported_member.roles.cache.has(WARNED_ROLE_1_ID)) {
                confirm = await prompt(client, message, `Do you confirm you want to report ${reported_user.username}, this will be his 2nd offense`, delay);
                level = 2;
            }
            else if (reported_member.roles.cache.has(WARNED_ROLE_2_ID)) {
                confirm = await prompt(client, message, `Do you confirm you want to report ${reported_user.username}, this will be his 3nd and last offense`, delay);
                level = 3;
            }
...
commands["cleanse"] = {
    ...
    command: async (client, message, args) => {
        ...
        const has_admin_role = author_member && author_member.roles.cache.has(ADMIN_ROLE_ID);
        if (!has_admin_role) {
            return consume(client, message, "Missing permissions", "You don't have the sufficient role to cleanse someone", "red");
        }
        let cleansed_user = null;
        if (!args[0]) {
            const who_answer = await prompt(client, message, `Looks like you forgot who to cleanse, who do you want to cleanse. @ him please.`, delay, true);
        ...
        try {
            ...
            if (cleansed_member.roles.cache.has(WARNED_ROLE_1_ID)) {
                confirm = await prompt(client, message, `Do you confirm you want to cleanse ${cleansed_user.username}, he is currently at his 1st offense and will be cleansed of all negative roles`, delay);
                level = 1;
            }
            else if (cleansed_member.roles.cache.has(WARNED_ROLE_2_ID)) {
                confirm = await prompt(client, message, `Do you confirm you want to cleanse ${cleansed_user.username}, he is currently at his 2nd offense and will be downgraded to the 1st offense`, delay);
                level = 2;
            }
...
commands["modregister"] = {
    ...
    command: (client, message, args) => {
        if (args.length < 2) {
            return consume(client, message, "Cannot register your mod", `<@${message.author.id}>, to register a mod you must give a name, a link and optionally a description for the mod.`, "red");
        }
        const [name, link, ...description] = args;
        mod_requests.set(args[0], new Mod(name, link, description.join(" ")));
        saveModDatabase();
        consume(client, message, "Thanks for registering a new mod", `<@${message.author.id}> has registered the mod ${args[0]}. It will be added to the database after a Peacekeeper has validater the request`, "blue");
    },
};
commands["modrequests"] = {
    ...
    command: (client, message, args) => {
        ...
        const has_admin_role = author_member && author_member.roles.cache.has(ADMIN_ROLE_ID);
        if (!has_admin_role) {
            return consume(client, message, "Missing permissions", "You don't have the sufficient role to view the pending requests", "red");
        }
        if (!args.length) {
            const pending_requests = Array.from(mod_requests.entries()).map(([name, { link }]) => `**${name}**: ${link}`);
            if (!pending_requests.length) {
                return consume(client, message, "No pending requests left", "No pending requests left", "red");
            }
            consume(client, message, "Here is a list of all the pending requests i have", pending_requests.join("\n") || "", "green");
        }
        else if (args.length == 1) {
            const pending_request = mod_requests.get(args[0]);
            consume(client, message, "Here is the information i have on the pending request", [
            ...
            if (!mod_requests.has(args[0])) {
                return consume(client, message, "Cannot accept pending request", `There is no pending request for ${args[0]}`, "red");
            }
            ...
            if (is_accepted) {
                ...
                saveModDatabase();
                return consume(client, message, "Pending request accepted", `The mod **${args[0]}** was accepted and has been added to the database`, "green");
            }
            consume(client, message, "Pending request denied", `The mod **${args[0]}** was denied and has been removed from the requests`, "red");
        }
    },
};
commands["mods"] = {
    ...
    command: (client, message, args) => {
        ...
        if (arg === "name") {
            if (!registered_mods.has(search.join(" ").trim())) {
                return consume(client, message, "No mod found", `There is no mod with the name ${search}`, "red");
            }
            ...
            consume(client, message, "Here is the information i have about the mod", mod_information, "green");
        }
        else if (arg === "search") {
            const mods = Array.from(registered_mods).filter(([name, { link, description }]) => search.some((s) => name.includes(s) || description.includes(s)));
            consume(client, message, "Here is the information i have about the mod. (descriptions removed)", mods.map(([_, mod]) => `**__${mod.name}__** - ${mod.link}`).join("\n"), "green");
        }
        else {
            const mods = Array.from(registered_mods);
            if (!mods.length) {
                return consume(client, message, "Here is a list of all the mods i know. (descriptions removed)", "No registered mod yet, use the `$modregister` command to add one", "red");
            }
            consume(client, message, "Here is a list of all the mods i know. (descriptions removed)", mods.map(([_, mod]) => `**__${mod.name}__** - ${mod.link}`).join("\n"), "green");
        }
    },
};
commands["modremove"] = {
    ...
    command: (client, message, args) => {
        ...
        const has_admin_role = author_member && author_member.roles.cache.has(ADMIN_ROLE_ID);
        if (!has_admin_role) {
            return consume(client, message, "Missing permissions", "You don't have the sufficient role to remove a mod", "red");
        }
        const [name] = args;
        if (!registered_mods.has(name)) {
            return consume(client, message, "No mod found", `No mod found with the name ${name}`, "red");
        }
        registered_mods.delete(name);
        saveModDatabase();
        return consume(client, message, "Mod removed", `the mod **${name}** was removed from the database`, "green");
    },
};
commands["say"] = {
    name: "say",
    ...
    command: async (client, message, args) => {
        if (args.length < 2) {
            return consume(client, message, "Invalid arguments", `you must provide a channel id and a message`, "red");
        }
...
commands["order66"] = {
    ...
    command: async (client, message, args) => {
        ...
        if (!start_message_id) {
            return consume(client, message, "Missing parameter", "You must pass as a second parameter the ID of the starting point for the deletion", "red");
        }
        const channel = channel_id.trim().toLowerCase() === "here"
            ? message.channel
            : await client.channels
                .fetch(channel_id)
                .catch(() => consume(client, message, "Unknown channel", `no channel exists with the id ${channel_id}`, "red"));
        const gif_message = await channel
            .send("https://tenor.com/view/execute-order66-order66-66-palpatine-star-wars-gif-20468321")
            .catch((e) => consume(client, message, "Error while sending a funny gif message", e, "red"));
        ...
        const messages = await channel.messages
            ...
            .catch((e) => consume(client, message, "Error while fetching the messages", e, "red"));
        if (end_message_id) {
        ...
        if (end_message_id) {
            gif_message
                .delete()
                .catch((e) => consume(client, message, "Error while deleting the gif message", e, "red"));
            await message
                .delete()
                .catch((e) => consume(client, message, "Error while deleting the command message", e, "red"));
        }
    },
};
commands["test"] = {
    ...
    if (message.content.startsWith(COMMANDS_PREFIX)) {
        ...
        if (command in commands) {
            try {
                ...
                return consume(client, message, "Error", `error when running command \`${command}\`: ${err}`, "red");
            }
        }
        return consume(client, message, "Error", `no such command \`${command}\``, "red");
    }
    /**
...
client.on("interactionCreate", async (interaction) => {
    ...
    (0, antibot_handler_1.antibot_interaction_handler)(interaction, client);
    (0, active_users_1.activeUserInteractionHandler)(interaction, client);
});
client.on("inviteCreate", antibot_handler_1.antibot_invite_create_handler);
